...
Associados ao Desempenho no ENEM 2024}
...
O Brasil apresenta profundas desigualdades educacionais e socioeconômicas que se refletem nos resultados do ENEM. Estudantes de escolas privadas, com maior acesso a recursos educacionais, tendem a obter desempenho superior àqueles de escolas públicas. Da mesma forma, a renda familiar — que determina o acesso a cursinhos, materiais didáticos e tempo disponível para estudo — é um fator fortemente associado ao desempenho no exame.

Este trabalho tem como objetivo analisar o perfil dos candidatos do ENEM 2024 e investigar a relação entre fatores socioeconômicos (tipo de escola e renda familiar) e o desempenho nas cinco áreas do conhecimento. A análise é conduzida por meio de um dashboard interativo desenvolvido com Streamlit e Plotly Express.
...
Q006 & Renda familiar mensal (A = nenhuma, B = até R\$ 1.320) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
    ...
    \label{fig:g1}
    \vspace{1cm}
    \textit{(Inserir print do gráfico G1 -- pizza)}
\end{figure}
...
\begin{figure}[H]
    ...
    \label{fig:g2}
    \vspace{1cm}
    \textit{(Inserir print do gráfico G2 -- barras horizontais)}
\end{figure}
...
\begin{figure}[H]
    ...
    \label{fig:g3}
    \vspace{1cm}
    \textit{(Inserir print do gráfico G3 -- barras verticais)}
\end{figure}
...
\begin{figure}[H]
    ...
    \label{fig:g4}
    \vspace{1cm}
    \textit{(Inserir print do gráfico G4 -- pizza)}
\end{figure}
...
\begin{figure}[H]
    ...
    \label{fig:g5}
    \vspace{1cm}
    \textit{(Inserir print do gráfico G5 -- barras agrupadas)}
\end{figure}
...
\begin{figure}[H]
    ...
    \label{fig:g6}
    \vspace{1cm}
    \textit{(Inserir print do gráfico G6 -- barras horizontais)}
\end{figure}
...
\begin{figure}[H]
    ...
    \label{fig:g7}
    \vspace{1cm}
    \textit{(Inserir print do gráfico G7 -- box plot)}
\end{figure}
...
\begin{figure}[H]
    ...
    \label{fig:g8}
    \vspace{1cm}
    \textit{(Inserir print do gráfico G8 -- barras agrupadas)}
\end{figure}
...
\begin{figure}[H]
    ...
    \label{fig:r11}
    \vspace{1cm}
    \textit{(Inserir print do gráfico R1.1 -- barras agrupadas)}
\end{figure}

\subsubsection{R1.2 -- Distribuição das Notas: Pública vs. Privada}

O box plot (Figura~\ref{fig:r12}) revela a dispersão das notas, evidenciando que escolas privadas apresentam maior variabilidade e medianas superiores.

\begin{figure}[H]
    \centering
    \caption{Distribuição das notas: pública vs. privada (R1.2)}
    \label{fig:r12}
    \vspace{1cm}
    \textit{(Inserir print do gráfico R1.2 -- box plot)}
\end{figure}
...
\begin{figure}[H]
    ...
    \label{fig:r13}
    \vspace{1cm}
    \textit{(Inserir print do gráfico R1.3 -- barras agrupadas)}
\end{figure}
...
\begin{figure}[H]
    ...
    \label{fig:r14}
    \vspace{1cm}
    \textit{(Inserir print do gráfico R1.4 -- histograma)}
\end{figure}
...
\subsection{RQ2 -- Renda Familiar e Desempenho}

\textbf{Pergunta:} Qual a relação entre a renda familiar mensal e o desempenho médio no ENEM 2024?

\textbf{Observação importante:} No ENEM 2024, a variável Q006 (renda familiar) apresenta apenas duas categorias: \textbf{A} (nenhuma renda) e \textbf{B} (até R\$ 1.320). Isso limita a granularidade da análise, mas ainda permite comparar candidatos sem renda familiar declarada com aqueles de baixa renda.

\subsubsection{R2.1 -- Relação entre Renda Familiar e Nota Média}

A Figura~\ref{fig:r21} mostra a mediana da nota média para cada faixa de renda. A diferença entre as duas faixas é pequena.

\begin{figure}[H]
    \centering
    \caption{Relação entre renda familiar e nota média (R2.1)}
    \label{fig:r21}
    \vspace{1cm}
    \textit{(Inserir print do gráfico R2.1 -- linha)}
\end{figure}

\subsubsection{R2.2 -- Mediana por Área e Renda (Heatmap)}

O heatmap (Figura~\ref{fig:r22}) mostra que a mediana das notas é praticamente estável entre as duas faixas de renda.

\begin{figure}[H]
    \centering
    \caption{Mediana por área e faixa de renda (R2.2)}
    \label{fig:r22}
    \vspace{1cm}
    \textit{(Inserir print do gráfico R2.2 -- heatmap)}
\end{figure}

\subsubsection{R2.3 -- Distribuição de Candidatos por Renda}

A maioria dos candidatos (68,8\%) declarou não ter renda familiar (faixa A), enquanto 31,2\% declararam renda de até R\$ 1.320 (faixa B), conforme a Figura~\ref{fig:r23}.

\begin{figure}[H]
    \centering
    \caption{Distribuição de candidatos por renda (R2.3)}
    \label{fig:r23}
    \vspace{1cm}
    \textit{(Inserir print do gráfico R2.3 -- barras verticais)}
\end{figure}

\subsubsection{R2.4 -- Mediana por Renda e Tipo de Escola}

A Figura~\ref{fig:r24} mostra a interação entre renda e tipo de escola. A Tabela~\ref{tab:rendaescola} apresenta os valores.

\begin{table}[H]
\centering
\caption{Mediana da nota média por renda e tipo de escola}
\label{tab:rendaescola}
\begin{tabular}{lcc}
\toprule
\textbf{Renda} & \textbf{Pública} & \textbf{Privada} \\
\midrule
Nenhuma renda    & 510,0 & 620,7 \\
Até R\$ 1.320  & 509,3 & 620,4 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
    \centering
    \caption{Mediana por renda e tipo de escola (R2.4)}
    \label{fig:r24}
    \vspace{1cm}
    \textit{(Inserir print do gráfico R2.4 -- barras agrupadas)}
\end{figure}

\textbf{Achados:}
\begin{itemize}
    \item A mediana da nota média é praticamente idêntica entre as duas faixas de renda: 528,7 (nenhuma renda) e 528,4 (até R\$ 1.320).
    \item O tipo de escola é um fator muito mais discriminante que a renda: dentro de cada faixa de renda, a diferença entre escolas públicas e privadas é de aproximadamente 110 pontos.
    \item Candidatos de escolas privadas com nenhuma renda (620,7) têm desempenho muito superior a candidatos de escolas públicas com a mesma renda (510,0).
\end{itemize}
...
Os dados revelam que o tipo de escola é fortemente associado ao desempenho no ENEM 2024, com diferenças expressivas em todas as áreas do conhecimento. A renda familiar, na forma como foi capturada neste ano (apenas duas faixas), apresenta baixa capacidade discriminatória quando analisada isoladamente.
...
A análise conjunta de renda e tipo de escola (R2.4) revela que, dentro de cada faixa de renda, a diferença entre escolas públicas e privadas se mantém elevada (cerca de 110 pontos na nota média). Isso sugere que o tipo de escola é um fator preditor mais forte que a renda \textit{per se}, embora as duas variáveis estejam correlacionadas: candidatos de escolas privadas tendem a pertencer a faixas de renda mais altas (quando mais faixas estivessem disponíveis).
...
\begin{itemize}
    ...
    \item \textbf{Renda autodeclarada}: a variável Q006 é auto-reportada e, no ENEM 2024, apresenta apenas duas categorias, limitando a análise de desigualdade de renda.
...
\begin{itemize}
    ...
    \item Inclusão de variáveis adicionais como escolaridade dos pais (Q001, Q002) para análise multivariada.
    \item Análise por raça/cor interseccionada com renda e tipo de escola para investigar desigualdades estruturais.
    \item Comparação com edições anteriores do ENEM que possuíam mais faixas de renda, permitindo análise mais granular.
\end{itemize}
