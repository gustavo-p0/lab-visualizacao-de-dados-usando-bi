...
Tipo de Escola, Região e Perfil dos Candidatos}
...
O Brasil apresenta profundas desigualdades educacionais e socioeconômicas que se refletem nos resultados do ENEM. Estudantes de escolas privadas, com maior acesso a recursos educacionais, tendem a obter desempenho superior àqueles de escolas públicas. Da mesma forma, a região geográfica do candidato pode influenciar o acesso a uma educação de qualidade e, consequentemente, o desempenho no exame.

Este trabalho tem como objetivo analisar o perfil dos candidatos do ENEM 2024 e investigar a relação entre fatores socioeconômicos (tipo de escola e região) e o desempenho nas cinco áreas do conhecimento. A análise é conduzida por meio de um dashboard interativo desenvolvido com Streamlit e Plotly Express.
...
\subsection{RQ2 -- Região e Desempenho}

\textbf{Pergunta:} Qual a relação entre a região geográfica do candidato e o desempenho no ENEM 2024?

\subsubsection{R2.1 -- Desvio da Nota Média Mediana por Região}

A Figura~\ref{fig:r21} apresenta o desvio da mediana de cada região em relação à mediana global. As diferenças são mínimas (menos de 1 ponto entre extremos), indicando que, neste recorte, o desempenho mediano é homogêneo entre as regiões.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{r21}
    \caption{Desvio da nota média mediana por região (R2.1)}
    \label{fig:r21}
\end{figure}

\subsubsection{R2.2 -- Mediana por Área e Região}

A Figura~\ref{fig:r22} confirma o padrão para cada área individualmente: as medianas são praticamente idênticas entre as regiões em todas as cinco áreas.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{r22}
    \caption{Mediana por área e região — facetas (R2.2)}
    \label{fig:r22}
\end{figure}

\subsubsection{R2.3 -- Distribuição de Candidatos por Região}

A Figura~\ref{fig:r23} mostra a distribuição regional dos candidatos. Nordeste e Sudeste concentram a maior parte da amostra.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{r23}
    \caption{Distribuição de candidatos por região (R2.3)}
    \label{fig:r23}
\end{figure}

\subsubsection{R2.4 -- Nota Média Mediana por Região e Tipo de Escola}

A Figura~\ref{fig:r24} cruza região com tipo de escola. O resultado mais expressivo é a \textbf{consistência nacional do gap público-privado}: em todas as regiões, escolas privadas têm mediana próxima de 620 e públicas próxima de 510.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{r24}
    \caption{Nota média mediana por região e tipo de escola (R2.4)}
    \label{fig:r24}
\end{figure}

\textbf{Achados:}
\begin{itemize}
    \item As diferenças regionais na mediana da nota média são inferiores a 1 ponto, indicando homogeneidade no desempenho entre regiões após os filtros aplicados.
    \item O gap entre escolas públicas e privadas é consistente em todas as regiões: privadas ~620, públicas ~510 em todo o Brasil.
    \item Isso sugere que a desigualdade educacional entre redes pública e privada é um fenômeno nacional, não regional.
    \item As diferenças regionais no ENEM, quando existem, provavelmente se manifestam mais na taxa de participação e presença do que no desempenho dos candidatos presentes em todas as provas.
\end{itemize}
...
A análise regional (RQ2) revela que, após os filtros de presença em todas as provas e exclusão de treineiros, as diferenças regionais no desempenho mediano são mínimas (menos de 1 ponto). O achado mais relevante é a consistência nacional do gap entre escolas públicas e privadas: em todas as regiões, a mediana das privadas é aproximadamente 620 e a das públicas 510.

\subsection{Relação entre RQ1 e RQ2}

Os resultados de RQ1 e RQ2, analisados em conjunto, indicam que o tipo de escola é o principal fator associado ao desempenho no ENEM 2024, e que esse efeito é homogêneo entre as regiões do país. A ausência de diferenças regionais significativas após o controle por presença e tipo de escola sugere que as desigualdades regionais na educação brasileira se manifestam principalmente no acesso e na permanência dos estudantes, e não no desempenho daqueles que conseguem concluir o ensino médio e realizar todas as provas.
...
\begin{itemize}
    ...
    \item \textbf{Dados autodeclarados}: variáveis como raça/cor e escolaridade dos pais são auto-reportadas e podem conter imprecisões.
...
\begin{itemize}
    ...
    \item Análise interseccional cruzando região, tipo de escola e faixa de renda para investigar desigualdades estruturais.
    \item Modelagem multivariada para quantificar a contribuição relativa de cada fator (escola, região, sexo) no desempenho.
\end{itemize}
