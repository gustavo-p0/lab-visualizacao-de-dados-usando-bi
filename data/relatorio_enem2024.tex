...
Tipo de Escola, Correlação entre Disciplinas e Perfil dos Candidatos}
...
\subsection{RQ2 -- Correlação entre Disciplinas}

\textbf{Pergunta:} Qual a correlação entre os desempenhos nas cinco áreas do conhecimento do ENEM 2024? Candidatos que vão bem em Ciências da Natureza também vão bem em Matemática? O desempenho em Redação é independente das provas objetivas?

\subsubsection{R2.1 -- Matriz de Correlação (Spearman)}

A Figura~\ref{fig:r21} apresenta a matriz de correlação de Spearman entre as cinco áreas. As correlações mais fortes são entre disciplinas afins: Ciências da Natureza e Ciências Humanas (0,64), Ciências da Natureza e Linguagens (0,63) e Matemática e Ciências da Natureza (0,60). Redação apresenta as menores correlações com as demais áreas (máximo de 0,58 com Ciências Humanas), indicando que a habilidade de escrita é relativamente independente do desempenho nas provas objetivas.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{r21}
    \caption{Matriz de correlação de Spearman entre disciplinas (R2.1)}
    \label{fig:r21}
\end{figure}

\subsubsection{R2.2 -- Correlações por Tipo de Escola}

A Tabela~\ref{tab:corr_publica} e a Tabela~\ref{tab:corr_privada} apresentam as matrizes de correlação separadas por tipo de escola. As correlações na rede privada são sistematicamente mais altas, com destaque para Matemática $\times$ Ciências da Natureza (0,73 na privada vs. 0,49 na pública) e Matemática $\times$ Ciências Humanas (0,62 vs. 0,49). Esse padrão sugere que o ensino privado produz um perfil de desempenho mais homogêneo.

\begin{table}[H]
\centering
\caption{Correlação (Spearman) — Escola Pública}
\label{tab:corr_publica}
\begin{tabular}{lccccc}
\toprule
 & CN & CH & LC & MT & Red \\
\midrule
CN & 1,00 & 0,54 & 0,54 & 0,49 & 0,35 \\
CH & 0,54 & 1,00 & 0,57 & 0,49 & 0,51 \\
LC & 0,54 & 0,57 & 1,00 & 0,46 & 0,52 \\
MT & 0,49 & 0,49 & 0,46 & 1,00 & 0,45 \\
Red & 0,35 & 0,51 & 0,52 & 0,45 & 1,00 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Correlação (Spearman) — Escola Privada}
\label{tab:corr_privada}
\begin{tabular}{lccccc}
\toprule
 & CN & CH & LC & MT & Red \\
\midrule
CN & 1,00 & 0,72 & 0,67 & 0,73 & 0,43 \\
CH & 0,72 & 1,00 & 0,67 & 0,62 & 0,52 \\
LC & 0,67 & 0,67 & 1,00 & 0,58 & 0,48 \\
MT & 0,73 & 0,62 & 0,58 & 1,00 & 0,52 \\
Red & 0,43 & 0,52 & 0,48 & 0,52 & 1,00 \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{R2.3 -- Dispersão: Matemática vs. Redação}

A Figura~\ref{fig:r23} apresenta a dispersão entre Matemática e Redação, as duas disciplinas mais distintas do ENEM. A correlação de Spearman é de apenas 0,54, a menor entre as objetivas e Redação. As linhas de tendência (OLS) confirmam que, embora a relação seja positiva, a variabilidade é alta — um mesmo desempenho em Matemática pode estar associado a notas muito diferentes em Redação.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{r23}
    \caption{Matemática vs. Redação com linha de tendência (R2.3)}
    \label{fig:r23}
\end{figure}

\subsubsection{R2.4 -- Dispersão: Matemática vs. Ciências da Natureza}

A Figura~\ref{fig:r24} mostra a correlação mais forte do ENEM: Matemática $\times$ Ciências da Natureza (0,61 no geral; 0,73 na privada vs. 0,49 na pública). As linhas de tendência deixam clara a diferença de inclinação entre os dois segmentos: na rede privada, o aumento em Matemática está associado a um aumento maior em CN.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{r24}
    \caption{Matemática vs. CN com linha de tendência por segmento (R2.4)}
    \label{fig:r24}
\end{figure}

\textbf{Achados:}
\begin{itemize}
    \item As correlações mais fortes são entre disciplinas afins: Ciências da Natureza e Ciências Humanas (0,64) e Ciências da Natureza e Linguagens (0,63).
    \item Redação é a disciplina mais independente, com correlação máxima de 0,58 com Ciências Humanas.
    \item Escolas privadas apresentam correlações sistematicamente mais altas entre todas as disciplinas. A maior diferença está em Matemática $\times$ Ciências da Natureza: 0,73 (privada) vs. 0,49 (pública).
    \item Esse padrão sugere que o ensino privado produz um perfil de desempenho mais homogêneo, enquanto na rede pública o desempenho é mais variável entre disciplinas.
\end{itemize}
...
\begin{itemize}
    ...
    \item Análise fatorial ou de componentes principais para identificar dimensões latentes de desempenho no ENEM.
    \item Modelagem multivariada para quantificar a contribuição relativa de cada fator (escola, sexo, região) no desempenho.
\end{itemize}
